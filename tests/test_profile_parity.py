...
def test_check_timestamp_parity(file_contents):
    """Verify that verification timestamps are synchronized to 2026-09-10."""
    assert "2026-09-10" in file_contents["README.md"]
    assert "<!-- last-checked: 2026-09-10 -->" in file_contents["profile/README.md"]
    assert "<!-- last-checked: 2026-09-10 -->" in file_contents["profile/README_de.md"]
    assert "## Last-checked: 2026-09-10" in file_contents["llms.txt"]
    assert "2026-09-10" in file_contents["CHANGELOG.md"]
...
def test_push_timestamp_parity(file_contents):
    ...
    for filename in ["README.md", "profile/README.md", "profile/README_de.md", "llms.txt", "CHANGELOG.md"]:
        ...
        assert "2026-08-20" in text or "20.08.2026" in text, f"Missing verordnungsampel push date in {filename}"
